function _tide_item_cmd_duration
    ...
    if test $CMD_DURATION -gt 0
        set -l milliseconds (math --scale=0 "$CMD_DURATION" % 1000)
        ...
        if test $hours != 0
            ...
            _tide_print_item cmd_duration $tide_cmd_duration_icon' ' $seconds's ' $milliseconds 'ms'
        else if test $milliseconds != 0 
            _tide_print_item cmd_duration $tide_cmd_duration_icon' ' $seconds's ' $milliseconds 'ms'
        end
    end
end
